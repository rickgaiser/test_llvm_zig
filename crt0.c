...
// LLVM-style init/fini arrays (function pointer arrays)
...
// Call all functions in an init/fini array
static void __call_array(init_func *start, init_func *end) {
...
// Call fini array in reverse order (LIFO)
static void __call_fini_array(init_func *start, init_func *end) {
    for (init_func *fn = end - 1; fn >= start; fn--) {
        if (*fn)
            (*fn)();
    }
}
...
// Override weak libcglue pthread glue functions (libpthreadglue disabled)
...
static void _main()
{
    ...
    // Call global constructors (LLVM-style init arrays)
    __call_array(__preinit_array_start, __preinit_array_end);
    __call_array(__init_array_start, __init_array_end);

    // call main
    retval = main(pa->argc, pa->argv);

    // Call global destructors (LLVM-style fini array, reverse order)
    __call_fini_array(__fini_array_start, __fini_array_end);
